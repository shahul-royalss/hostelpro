...
create policy users_select on public.users for select
  using (
    ...
    or (app.user_role() in ('owner','warden') and app.can_read_hostel(hostel_id))
    -- the Manager needs staff for tasks/announcements, but must not enumerate residents
    or (app.user_role() = 'manager' and app.can_read_hostel(hostel_id) and role in ('owner','manager','warden'))
  );
...
-- §4.8: a student could otherwise read every bed row, including beds.student_id, mapping
-- each occupied bed to another resident's UUID. Students see only their own room.
create policy beds_select on public.beds for select
  using (
    app.is_staff_of(hostel_id)
    or room_id = (select room_id from public.students where user_id = auth.uid() and status <> 'vacated' limit 1)
  );
...
-- LEAST PRIVILEGE (checklist §28): app.is_staff_of() includes 'manager', which let the
-- Manager read every resident's PII, fees, leaves, visitors and complaints. CLAUDE_2.md
-- §6.3 scopes the Manager to expenses, revenue, tasks, mess menu and the announcements
-- feed — no manager query, action or page touches a student table. Resident data is
-- therefore limited to warden + owner (and the student's own rows).
create policy students_select on public.students for select
  using (app.has_role_in(hostel_id, 'warden', 'owner') or user_id = auth.uid());
...
create policy fees_select on public.fee_payments for select
  using (app.has_role_in(hostel_id, 'warden', 'owner') or student_id = app.current_student_id());
...
create policy complaints_select on public.complaints for select
  using (app.has_role_in(hostel_id, 'owner', 'warden') or student_id = app.current_student_id());
...
create policy complaint_events_select on public.complaint_events for select
  using (
    app.has_role_in(hostel_id, 'owner', 'warden')
