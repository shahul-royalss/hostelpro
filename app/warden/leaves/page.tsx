...
import { getLeaves, getVisitors } from "@/lib/queries/warden";
...
export default async function WardenLeavesPage() {
  ...
  const [leaves, todaysVisitors] = await Promise.all([getLeaves(supabase, ctx.hostel.id, 80), getVisitors(supabase, ctx.hostel.id, startOfToday.toISOString(), 50)]);
