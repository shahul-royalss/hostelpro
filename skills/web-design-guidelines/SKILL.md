...
The rules live in `guidelines-snapshot.md` in this skill's directory — a vendored snapshot of https://raw.githubusercontent.com/vercel-labs/web-interface-guidelines/main/command.md (pinned at install time, 2026-08-01, so review behavior is deterministic and works offline). Only refresh the snapshot if the user explicitly asks to update the guidelines.
