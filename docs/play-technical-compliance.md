...
```
$ aapt2 dump badging binary.apk
package: name='app.nivora.twa' versionCode='1' versionName='1.0.0'
...
application: label='NIVORA' icon='res/mipmap-anydpi-v26/ic_launcher.xml'
launchable-activity: name='app.nivora.twa.LauncherActivity'  label='NIVORA' icon=''
```
...
| Launcher activity | `app.nivora.twa.LauncherActivity` |
...
of `app.nivora.twa`, it must change **before** the first upload, in
...
permission: app.nivora.twa.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION
uses-permission: name='app.nivora.twa.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION'
```
...
| `app.nivora.twa.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION` | `signature` (`protectionLevel=0x2`) | **Yes.** Auto-generated by `androidx.core` when it registers a non-exported runtime receiver on API 33+. It is namespaced to this app, `signature`-level so only a package signed with the same key can hold it, and it grants access to nothing outside the app. Not shown to users; nothing to declare in Data safety. |
...
Certificate #1:
Owner: CN=NIVORA, O=NIVORA, C=IN
Issuer: CN=NIVORA, O=NIVORA, C=IN
...
Number of signers: 1
V3.0 Signer: certificate DN: CN=NIVORA, O=NIVORA, C=IN
...
&relation=delegate_permission/common.handle_all_urls\
&target.android_app.package_name=app.nivora.twa\
...
| Certificate DN is `CN=NIVORA, O=NIVORA, C=IN` | Not user-visible, and under Play App Signing not even the certificate devices see. | Fine as is; change to the registered entity name only if preferred. |
...
curl -sS "https://digitalassetlinks.googleapis.com/v1/assetlinks:check?source.web.site=https://hostelpro-three.vercel.app&relation=delegate_permission/common.handle_all_urls&target.android_app.package_name=app.nivora.twa&target.android_app.certificate.sha256_fingerprint=<sha256>"
